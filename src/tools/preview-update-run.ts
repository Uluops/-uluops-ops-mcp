 ...
 *
 * Read-only preview of an analysis-bearing update_run (API 1a:
 * POST /runs/:id/update-preview and POST /runs/update-preview). Computes,
 * per agent named in the payload, what a replace write would supersede,
 * create, and retire by omission — without writing anything.
 *
...
export function registerPreviewUpdateRunTool(
  ...
  server.tool(
    'preview_update_run',
    'Read-only preview of an analysis-bearing update_run: reports, per agent named in the payload, what a PER-AGENT REPLACE write would supersede, create, and retire by omission (would_retire_record_ids — the named agents\' live records absent from the payload). Nothing is written. Accepts analysis concerns only; any other update field is rejected by name. Identify run by either run_id OR (project + run_number).',
